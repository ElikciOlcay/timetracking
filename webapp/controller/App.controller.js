sap.ui.define([
	...
	return BaseController.extend("Zeiterfassung.controller.App", {
		...
		onInit: function () {
			...
			setTimeout(function () {
				self.getModel("nav").setProperty("/busy", false);
			}, 2000);

			window.addEventListener('touchstart', this.setSessionTimeout(this), false);
			window.addEventListener('click', this.setSessionTimeout(this), false);
			window.addEventListener('keypress', this.setSessionTimeout(this), false);
			this.sessionTimer = undefined;

		},

		setSessionTimeout: function (context) {
			var router = context.getRouter();
			if (this.sessionTimer !== undefined) {
				this.sessionTimer = clearTimeout(this.sessionTimer);
			}
			this.sessionTimer = setTimeout(() => {
				context.fixedSizeDialog = new Dialog({
		...
		onItemSelect: function (oEvent) {
			...
			if ((sKey === "home" || sKey === "master" || sKey === "statistics" || sKey === "others")) {
				// if the device is phone, collaps the navigation side of the app to give more space
				if (Device.system.phone) {
					this.onSideNavButtonPress();
				}
				if (sKey === "master") {
					sap.ui.getCore().getModel("pageModel").setProperty("/pageId", 1);
